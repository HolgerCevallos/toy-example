...
Rcpp::List findpcs2(arma::mat x, arma::mat B, arma::rowvec mu, int h, double s, 
...
//      sc0 = std::numeric_limits<double>::infinity();
      if (s == 0) {sc0 = sqrt( sum(sqresid_dist(x_index.head(h))) / h );}
      else {sc0 = s;}
...
//      Rcpp::Rcout << "B is " << B << std::endl;   
      // x_index = sort_index(sqresid_dist); // NUEVO!
      s = sqrt( sum(sqresid_dist(x_index.head(h))) / h ); 
      delta1 = 1-s/s0;
      s0 = s;
//      Rcpp::Rcout << "The scale is " << s << std::endl;      
  ...
  // x_index = x_index.head(h)+1; // NUEVO!
  B = Bprov.t(); // NUEVO!
//  return(B);
return Rcpp::List::create(Rcpp::Named("Bmat") = B, Rcpp::Named("mu") = mu, Rcpp::Named("scale") = s); // NUEVO!
                          // Rcpp::Named("index") = x_index.t()); // NUEVO!
